...
For each session the proxy connects to Telegram using a raw TLS WebSocket upgrade. The target URL follows Telegram's own WebSocket domain convention:

```
wss://kws{dc}.web.telegram.org/apiws
```

For media DCs and as a secondary domain, `kws{dc}-1.web.telegram.org` is also tried. The TCP connection is made directly to the DC's IP address (configured via `--dc-ip`), but the TLS SNI and the `Host` header carry the `kws{dc}.web.telegram.org` domain name. Once the WebSocket handshake is complete the proxy sends the 64-byte relay init packet and starts the bidirectional bridge.

<Tip>
The WS upgrade path is always `/apiws`. Telegram's WebSocket endpoints require this specific path and reject anything else.
</Tip>
