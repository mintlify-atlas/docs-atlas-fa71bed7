...
Returns a linkable host string from the configured bind address. When `host` is `"0.0.0.0"` or `"::"` it returns `"127.0.0.1"` so that the generated `tg://proxy` link points to localhost.
...
Programmatic entry point. Calls `asyncio.run(_run(stop_event))`. Pass an `asyncio.Event` to stop the server gracefully from another thread (GUI apps set this event on window close).
